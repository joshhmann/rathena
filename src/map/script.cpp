...
static bool playerbot_cfg_is_set(const std::string& key) {
	return mapreg_readreg(reference_uid(add_str(("$PBCFG_SET_" + key).c_str()), 0)) != 0;
}

static int64 playerbot_cfg_get_int(const std::string& key, int64 default_value) {
	if (!playerbot_cfg_is_set(key))
		return default_value;
	return mapreg_readreg(reference_uid(add_str(("$PBCFG_" + key).c_str()), 0));
}

static std::string playerbot_cfg_get_str(const std::string& key, const char* default_value = "") {
	if (!playerbot_cfg_is_set(key))
		return default_value;
	char* value = mapreg_readregstr(reference_uid(add_str(("$PBCFG_" + key + "$").c_str()), 0));
	return value != nullptr ? value : default_value;
}

static std::string playerbot_make_login_name(const char* bot_key) {
	std::string out = "pb_";

	for (const unsigned char* p = reinterpret_cast<const unsigned char*>(bot_key); *p != '\0'; ++p) {
		unsigned char c = *p;
		if (std::isalnum(c))
...
static std::string playerbot_template_alias(const char* template_key) {
	if (template_key == nullptr)
		return "";
	if (strcmp(template_key, "social.prontera.day") == 0)
		return "spd";
	if (strcmp(template_key, "social.alberta.night") == 0)
		return "san";
...
static bool playerbot_profile_lookup_by_key(const char* bot_key, uint32* bot_id) {
	char esc_key[129];
	...
	if (SQL_ERROR == Sql_Query(mmysql_handle,
		"SELECT `bot_id` FROM `bot_profile` WHERE `bot_key` = '%s' LIMIT 1",
		esc_key)) {
...
static std::string playerbot_buyingstore_state(const map_session_data* sd) {
	if (sd == nullptr)
		return "offline";

	return "open=" + std::to_string(sd->state.buyingstore ? 1 : 0)
		+ ",buyer_id=" + std::to_string(sd->buyer_id)
		+ ",slots=" + std::to_string(sd->buyingstore.slots)
...
static bool playerbot_buyinglist_refresh(map_session_data* viewer_sd, map_session_data* buyer_sd, bool reopen) {
	if (viewer_sd == nullptr || buyer_sd == nullptr || !buyer_sd->state.buyingstore || buyer_sd->buyer_id == 0 || buyer_sd->buyingstore.slots == 0)
		return false;

	if (reopen) {
		if (viewer_sd->searchstore.open)
			searchstore_close(*viewer_sd);
		if (!searchstore_open(*viewer_sd, 1, SEARCHSTORE_EFFECT_REMOTE, viewer_sd->m))
			return false;
	}

	viewer_sd->searchstore.type = SEARCHTYPE_BUYING_STORE;
	viewer_sd->searchstore.remote_id = buyer_sd->status.account_id;
	viewer_sd->searchstore.pages = 0;
	viewer_sd->searchstore.items.clear();

	for (uint32 i = 0; i < buyer_sd->buyingstore.slots && i < MAX_BUYINGSTORE_SLOTS; ++i) {
		if (buyer_sd->buyingstore.items[i].nameid == 0 || buyer_sd->buyingstore.items[i].amount == 0)
			continue;
		...
		ssitem->nameid = buyer_sd->buyingstore.items[i].nameid;
		ssitem->amount = buyer_sd->buyingstore.items[i].amount;
		ssitem->price = buyer_sd->buyingstore.items[i].price;
		memset(ssitem->card, 0, sizeof(ssitem->card));
		ssitem->refine = 0;
		ssitem->enchantgrade = 0;
		viewer_sd->searchstore.items.push_back(ssitem);
	}

	if (reopen)
		buyingstore_open(viewer_sd, buyer_sd->status.account_id);

	return viewer_sd->searchstore.open
		&& viewer_sd->searchstore.type == SEARCHTYPE_BUYING_STORE
		&& viewer_sd->searchstore.remote_id == static_cast<int32>(buyer_sd->status.account_id)
		&& !viewer_sd->searchstore.items.empty();
}
...
static int32 playerbot_cart_amount(const map_session_data* sd, t_itemid nameid) {
	...
	for (int32 i = 0; i < MAX_CART; ++i) {
		if (sd->cart.u.items_cart[i].nameid != nameid)
			continue;
		total += sd->cart.u.items_cart[i].amount;
	}

	return total;
}

static const char* playerbot_additem_result_name(e_additem_result flag) {
	switch (flag) {
		case ADDITEM_SUCCESS: return "success";
		case ADDITEM_INVALID: return "invalid";
		case ADDITEM_OVERWEIGHT: return "overweight";
		case ADDITEM_ITEM: return "item_limit";
		case ADDITEM_OVERITEM: return "inventory_limit";
		case ADDITEM_OVERAMOUNT: return "overamount";
		case ADDITEM_REFUSED_TIME: return "refused_time";
		case ADDITEM_STACKLIMIT: return "stacklimit";
...
static bool playerbot_session_mode_valid(const char* mode) {
	...
	static const char* const modes[] = {
		"progress", "menuskill", "skillitem", "searchstore", "mail",
		"roulette", "enchantgrade", "reform", "refine", "stylist",
		"expand", "barter", "laphine", "bank", "all"
	};
...
static bool playerbot_session_mode_active(const map_session_data* sd, const char* mode) {
	if (sd == nullptr || mode == nullptr)
		return false;

	if (strcmp(mode, "all") == 0)
		return playerbot_session_count(sd) > 0;
	if (strcmp(mode, "progress") == 0)
		return sd->progressbar.npc_id != 0;
	...
	if (strcmp(mode, "mail") == 0)
		return sd->state.mail_writing;
	if (strcmp(mode, "roulette") == 0)
		return sd->state.roulette_open;
	if (strcmp(mode, "enchantgrade") == 0)
		return sd->state.enchantgrade_open;
	if (strcmp(mode, "reform") == 0)
		return (sd->state.item_reform != 0 || sd->state.item_enchant_index != 0);
	if (strcmp(mode, "refine") == 0)
		return sd->state.refineui_open;
	if (strcmp(mode, "stylist") == 0)
		return sd->state.stylist_open;
	if (strcmp(mode, "expand") == 0)
		return sd->state.inventory_expansion_confirmation != 0;
	if (strcmp(mode, "barter") == 0)
		return (sd->state.barter_open || sd->state.barter_extended_open);
	if (strcmp(mode, "laphine") == 0)
		return (sd->state.laphine_synthesis != 0 || sd->state.laphine_upgrade != 0);
	if (strcmp(mode, "bank") == 0)
		return sd->state.banking;

	return false;
}

static bool playerbot_session_mode_set(map_session_data* sd, const char* mode, bool enable) {
	if (sd == nullptr || !playerbot_session_mode_valid(mode))
		return false;
	...
	if (strcmp(mode, "all") == 0) {
		...
		if (playerbot_session_mode_set(sd, "progress", true)
			&& playerbot_session_mode_set(sd, "menuskill", true)
			&& playerbot_session_mode_set(sd, "skillitem", true)
			&& playerbot_session_mode_set(sd, "searchstore", true)
			&& playerbot_session_mode_set(sd, "mail", true)
			&& playerbot_session_mode_set(sd, "roulette", true)
			&& playerbot_session_mode_set(sd, "enchantgrade", true)
			&& playerbot_session_mode_set(sd, "reform", true)
			&& playerbot_session_mode_set(sd, "refine", true)
			&& playerbot_session_mode_set(sd, "stylist", true)
			&& playerbot_session_mode_set(sd, "expand", true)
			&& playerbot_session_mode_set(sd, "barter", true)
			&& playerbot_session_mode_set(sd, "laphine", true)
			&& playerbot_session_mode_set(sd, "bank", true))
			return playerbot_session_count(sd) > 0;
		return false;
	}
	...
	if (strcmp(mode, "menuskill") == 0) {
		if (enable) {
			...
			sd->menuskill_val = 1;
			sd->menuskill_val2 = 0;
		} else if (sd->menuskill_id != 0 || sd->menuskill_val != 0 || sd->menuskill_val2 != 0) {
			clif_menuskill_clear(sd);
		}
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	...
	if (strcmp(mode, "mail") == 0) {
		sd->state.mail_writing = enable;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "roulette") == 0) {
		sd->state.roulette_open = enable;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "enchantgrade") == 0) {
		sd->state.enchantgrade_open = enable;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "reform") == 0) {
		if (enable) {
	...
	if (strcmp(mode, "refine") == 0) {
		sd->state.refineui_open = enable;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "stylist") == 0) {
		sd->state.stylist_open = enable;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "expand") == 0) {
		sd->state.inventory_expansion_confirmation = enable ? 1 : 0;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "barter") == 0) {
		sd->state.barter_open = enable;
		sd->state.barter_extended_open = enable;
		return playerbot_session_mode_active(sd, mode) == enable;
	}
	if (strcmp(mode, "laphine") == 0) {
		if (enable) {
...
static void playerbot_cleanup_provisioned(uint32 account_id, uint32 char_id, uint32 bot_id) {
	if (bot_id != 0) {
		...
		Sql_Query(mmysql_handle, "DELETE FROM `bot_identity_link` WHERE `bot_id` = '%u'", bot_id);
		Sql_Query(mmysql_handle, "DELETE FROM `bot_profile` WHERE `bot_id` = '%u'", bot_id);
	}

	if (char_id != 0)
		Sql_Query(mmysql_handle, "DELETE FROM `char` WHERE `char_id` = '%u' LIMIT 1", char_id);
	if (account_id != 0)
		Sql_Query(mmysql_handle, "DELETE FROM `login` WHERE `account_id` = '%u' LIMIT 1", account_id);
}

static uint32 playerbot_provision_identity(const char* bot_key, const char* display_name, const char* template_key) {
	const std::string login_name = playerbot_make_login_name(bot_key);
	const std::string password = login_name;
	const std::string template_alias = playerbot_template_alias(template_key);
	const std::string key_prefix = std::string("tp.") + template_alias + ".";
	const std::string role = playerbot_cfg_get_str(key_prefix + "rl");
	const std::string home_map = playerbot_cfg_get_str(key_prefix + "hm");
	const std::string pool_key = playerbot_cfg_get_str(key_prefix + "pk");
	const std::string profile_key = playerbot_cfg_get_str(key_prefix + "pf");
	const std::string routine_group = playerbot_cfg_get_str(key_prefix + "rg");
	const std::string pulse_profile = playerbot_cfg_get_str(key_prefix + "pp");
	const std::string interaction_policy = playerbot_cfg_get_str(key_prefix + "ip", "ambient_only");
	const std::string party_policy = playerbot_cfg_get_str(key_prefix + "py", "never");
	const std::string presence_policy = playerbot_cfg_get_str(key_prefix + "ps", "demand_gated");
	...
	const uint16 cloth_color = static_cast<uint16>(cap_value(playerbot_cfg_get_int(key_prefix + "cc", 0), MIN_CLOTH_COLOR, MAX_CLOTH_COLOR));
	char esc_bot_key[129], esc_display[NAME_LENGTH * 2 + 1], esc_login[48], esc_pass[48], esc_email[96];
	...
	Sql_EscapeStringLen(mmysql_handle, esc_bot_key, bot_key, strnlen(bot_key, 64));
	Sql_EscapeStringLen(mmysql_handle, esc_display, display_name, strnlen(display_name, NAME_LENGTH - 1));
	Sql_EscapeStringLen(mmysql_handle, esc_login, login_name.c_str(), login_name.size());
	Sql_EscapeStringLen(mmysql_handle, esc_pass, password.c_str(), password.size());
	Sql_EscapeStringLen(mmysql_handle, esc_email, (login_name + "@bot.local").c_str(), login_name.size() + 10);
	Sql_EscapeStringLen(mmysql_handle, esc_role, role.c_str(), role.size());
	Sql_EscapeStringLen(mmysql_handle, esc_home_map, home_map.c_str(), home_map.size());
	Sql_EscapeStringLen(mmysql_handle, esc_pool_key, pool_key.c_str(), pool_key.size());
	Sql_EscapeStringLen(mmysql_handle, esc_profile_key, profile_key.c_str(), profile_key.size());
	Sql_EscapeStringLen(mmysql_handle, esc_routine_group, routine_group.c_str(), routine_group.size());
	Sql_EscapeStringLen(mmysql_handle, esc_pulse_profile, pulse_profile.c_str(), pulse_profile.size());
	Sql_EscapeStringLen(mmysql_handle, esc_interaction, interaction_policy.c_str(), interaction_policy.size());
	Sql_EscapeStringLen(mmysql_handle, esc_party_policy, party_policy.c_str(), party_policy.size());
	Sql_EscapeStringLen(mmysql_handle, esc_presence_policy, presence_policy.c_str(), presence_policy.size());
	...
	if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT 1 FROM `char` WHERE `name` = '%s' LIMIT 1", esc_display)) {
		Sql_ShowDebug(mmysql_handle);
		return 0;
	}
	if (Sql_NextRow(mmysql_handle) == SQL_SUCCESS) {
	...
	if (SQL_ERROR == Sql_Query(mmysql_handle,
		...
		"VALUES ('%s', '%s', '%c', '%s', '0', '0', '9', '', '0', '0', '0')",
		esc_login, esc_pass, sex, esc_email)) {
		Sql_ShowDebug(mmysql_handle);
		return 0;
	}
	...
	if (account_id == 0 || SQL_ERROR == Sql_Query(mmysql_handle,
		...
		"VALUES ('%u', '0', '%s', '%u', '1', '1', '0', '0', '1', '1', '1', '1', '1', '1', '40', '40', '11', '11', '%u', '%u', '%u', '%u', '%s', '%u', '%u', '%s', '%u', '%u', '%c')",
		account_id, esc_display, job_id, hair_style, hair_color, cloth_color, job_id, esc_home_map, home_x, home_y, esc_home_map, home_x, home_y, sex)) {
		Sql_ShowDebug(mmysql_handle);
		playerbot_cleanup_provisioned(account_id, 0, 0);
		return 0;
	}
	...
	if (char_id == 0 || SQL_ERROR == Sql_Query(mmysql_handle,
		...
		esc_bot_key, esc_display, esc_role, esc_home_map, esc_pool_key, esc_timezone_policy, esc_personality)) {
		Sql_ShowDebug(mmysql_handle);
		playerbot_cleanup_provisioned(account_id, char_id, 0);
		return 0;
	}
	...
	if (bot_id == 0
		|| SQL_ERROR == Sql_Query(mmysql_handle,
			"INSERT INTO `bot_identity_link` (`bot_id`, `account_id`, `char_id`, `link_status`, `linked_at`) "
			"VALUES ('%u', '%u', '%u', 'linked', NOW())",
			bot_id, account_id, char_id)
		|| SQL_ERROR == Sql_Query(mmysql_handle,
			"INSERT INTO `bot_appearance` (`bot_id`, `job_id`, `sex`, `hair_style`, `hair_color`, `cloth_color`, `weapon_view`, `shield_view`, `head_top`, `head_mid`, `head_bottom`, `robe`) "
			"VALUES ('%u', '%u', '%c', '%u', '%u', '%u', '0', '0', '0', '0', '0', '0')",
			bot_id, job_id, sex, hair_style, hair_color, cloth_color)
		|| SQL_ERROR == Sql_Query(mmysql_handle,
			"INSERT INTO `bot_runtime_state` (`bot_id`, `current_map`, `current_x`, `current_y`, `current_state`, `park_state`, `spawned_gid`, `despawn_grace_until`, `last_spawned_at`, `last_despawned_at`, `last_parked_at`, `last_route_key`, `last_seen_tick`) "
			"VALUES ('%u', '%s', '%u', '%u', 'offline', 'parked', NULL, NULL, NULL, NULL, NOW(), '', '0')",
			bot_id, esc_home_map, home_x, home_y)
		|| SQL_ERROR == Sql_Query(mmysql_handle,
	...
	ShowInfo("playerbot: provisioned bot '%s' (%u) linked to account %u / char %u with template '%s'.\n",
		bot_key, bot_id, account_id, char_id, template_key);
	return bot_id;
}
...
/*==========================================
 * Query one playerbot profile by stable bot_key.
 *------------------------------------------*/
BUILDIN_FUNC(playerbot_profile)
{
	uint32 bot_id = 0;

	if (playerbot_profile_lookup_by_key(script_getstr(st, 2), &bot_id))
		script_pushint(st, bot_id);
	else
		script_pushint(st, 0);
	return SCRIPT_CMD_SUCCESS;
}
...
/*==========================================
 * Provision one persistent playerbot identity from a template.
 *------------------------------------------*/
BUILDIN_FUNC(playerbot_provision)
{
	const char* bot_key = script_getstr(st, 2);
	const char* display_name = script_getstr(st, 3);
	const char* template_key = script_getstr(st, 4);
	uint32 bot_id = playerbot_provision_identity(bot_key, display_name, template_key);
...
/*==========================================
 * Invite one local playerbot/headless target into the attached player's party.
 *------------------------------------------*/
BUILDIN_FUNC(playerbot_partyinvite)
{
	map_session_data* sd = nullptr;
	...
	script_pushint(st, (tsd != nullptr && party_invite(*sd, tsd)) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

/*==========================================
 * Read the local party_id for one active playerbot/headless target.
 *------------------------------------------*/
BUILDIN_FUNC(playerbot_partyid)
{
	TBL_PC* tsd = map_charid2sd(script_getnum(st, 2));
	script_pushint(st, tsd != nullptr ? tsd->status.party_id : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(playerbot_cartset)
{
	const char* bot_key = script_getstr(st, 2);
	int32 type = script_getnum(st, 3);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	...
	if (sd == nullptr || type < 0 || type > MAX_CARTS) {
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	bool before_active = static_cast<bool>(pc_iscarton(sd));
	bool ok = pc_setcart(sd, type);
	bool after_active = static_cast<bool>(pc_iscarton(sd));
	if (type > 0 && ok && !after_active) {
		ShowWarning("buildin_playerbot_cartset: bot '%s' cart did not activate (char_id=%u, before=%d, after=%d, state.active=%d, pc_loaded=%d, fd=%d, prev=%p, class=%u, push=%d, sc_option=%d, status_option=%d).\n",
			bot_key != nullptr ? bot_key : "",
			char_id,
			before_active ? 1 : 0,
			after_active ? 1 : 0,
			sd->state.active ? 1 : 0,
...
BUILDIN_FUNC(playerbot_cartclear)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	...
	if (!static_cast<bool>(pc_iscarton(sd))) {
		script_pushint(st, 1);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(playerbot_cartgrant)
{
	const char* bot_key = script_getstr(st, 2);
	t_itemid nameid = script_getnum(st, 3);
	int32 requested = script_getnum(st, 4);
	...
	if (!static_cast<bool>(pc_iscarton(sd))) {
		ShowWarning("buildin_playerbot_cartgrant: bot '%s' has no active cart (char_id=%u, weight=%d/%d, cart_items=%d).\n",
			bot_key != nullptr ? bot_key : "", char_id, sd->cart_weight, sd->cart_weight_max, sd->cart_num);
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "cart", std::to_string(nameid).c_str(), "script.busy", "denied", "cart.grant", "cart.inactive");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	it.identify = 1;
	it.bound = BOUND_NONE;
	e_additem_result flag = pc_cart_additem(sd, &it, amount, LOG_TYPE_SCRIPT);
	if (flag != ADDITEM_SUCCESS) {
		ShowWarning("buildin_playerbot_cartgrant: bot '%s' failed to add item %u x%u to cart (char_id=%u, reason=%s, weight=%d/%d, cart_items=%d).\n",
			bot_key != nullptr ? bot_key : "", nameid, amount, char_id, playerbot_additem_result_name(flag), sd->cart_weight, sd->cart_weight_max, sd->cart_num);
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "cart", std::to_string(nameid).c_str(), "script.busy", "aborted", "cart.grant", playerbot_additem_result_name(flag));
	}
	script_pushint(st, flag == ADDITEM_SUCCESS ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_cartcount)
{
	const char* bot_key = script_getstr(st, 2);
	t_itemid nameid = script_getnum(st, 3);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	script_pushint(st, playerbot_cart_amount(sd, nameid));
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_cartactive)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	script_pushint(st, (sd != nullptr && static_cast<bool>(pc_iscarton(sd))) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_pcloaded)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	script_pushint(st, (sd != nullptr && sd->state.pc_loaded) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_vitalsrestore)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	...
	uint32 hp = sd->battle_status.max_hp > 0 ? static_cast<uint32>(sd->battle_status.max_hp) : sd->status.max_hp;
	uint32 sp = sd->battle_status.max_sp > 0 ? static_cast<uint32>(sd->battle_status.max_sp) : sd->status.max_sp;
	bool changed = false;

	if (hp > 0 && sd->battle_status.hp <= 0) {
		sd->battle_status.hp = static_cast<int32>(hp);
		sd->status.hp = hp;
		clif_updatestatus(*sd, SP_HP);
		changed = true;
	}
	if (sp > 0 && sd->battle_status.sp == 0 && sd->status.sp == 0) {
		sd->battle_status.sp = static_cast<int32>(sp);
		sd->status.sp = sp;
	...
	if (changed) {
		ShowInfo("playerbot: restored vitals for bot '%s' (%u:%u) to hp=%d/%d sp=%d/%d.\n",
			bot_key != nullptr ? bot_key : "",
			account_id,
			char_id,
			sd->battle_status.hp,
			...
			sd->battle_status.sp,
			sd->battle_status.max_sp);
	}

	script_pushint(st, (sd->battle_status.hp > 0) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(playerbot_sessionarm)
{
	...
	bool ok = playerbot_session_mode_set(sd, mode, true);
...
BUILDIN_FUNC(playerbot_sessionopen)
{
	const char* bot_key = script_getstr(st, 2);
	const char* mode = script_getstr(st, 3);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.requested", "session", mode, "operator.start", "ok", "", "");

	if (sd == nullptr || !playerbot_session_mode_valid(mode)) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "session", mode, "target.invalid", "denied", "session.open", sd == nullptr ? "bot.offline" : "mode.invalid");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	bool ok = playerbot_session_mode_set(sd, mode, true);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, ok ? "interaction.completed" : "interaction.failed", "session", mode, ok ? "operator.start" : "target.invalid", ok ? "ok" : "denied", ok ? "" : "session.open", playerbot_session_state(sd).c_str());
	script_pushint(st, ok ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_sessionclose)
{
	const char* bot_key = script_getstr(st, 2);
	const char* mode = script_getstr(st, 3);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.requested", "session", mode, "operator.stop", "ok", "", "");

	if (sd == nullptr || !playerbot_session_mode_valid(mode)) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "session", mode, "target.invalid", "denied", "session.close", sd == nullptr ? "bot.offline" : "mode.invalid");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	bool ok = playerbot_session_mode_set(sd, mode, false);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, ok ? "interaction.completed" : "interaction.failed", "session", mode, ok ? "operator.stop" : "target.invalid", ok ? "ok" : "denied", ok ? "" : "session.close", playerbot_session_state(sd).c_str());
	script_pushint(st, ok ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_sessionactive)
{
	const char* bot_key = script_getstr(st, 2);
	const char* mode = script_getstr(st, 3);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
...
BUILDIN_FUNC(playerbot_buyingstoreopen)
{
	const char* bot_key = script_getstr(st, 2);
	const char* title = script_getstr(st, 3);
	t_itemid nameid = static_cast<t_itemid>(script_getnum(st, 4));
	int32 amount = script_getnum(st, 5);
	int32 price = script_getnum(st, 6);
	int32 zenylimit = script_getnum(st, 7);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.requested", "buyingstore", title, "operator.start", "ok", "", "");

	if (sd == nullptr || nameid == 0 || amount <= 0 || price <= 0 || zenylimit <= 0) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "buyingstore", title, "target.invalid", "denied", "buyingstore.open", sd == nullptr ? "bot.offline" : "buyingstore.invalid");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	if (sd->state.buyingstore) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.completed", "buyingstore", title, "operator.start", "noop", "", "already.open");
		script_pushint(st, 1);
		return SCRIPT_CMD_SUCCESS;
	}

	int8 setup = buyingstore_setup(sd, 1);
	if (setup != 0) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "buyingstore", title, "target.invalid", "denied", "buyingstore.open", ("setup=" + std::to_string(setup)).c_str());
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	PACKET_CZ_REQ_OPEN_BUYING_STORE_sub item = {};
	item.itemId = static_cast<uint32>(nameid);
	item.amount = static_cast<uint16>(amount);
	item.price = static_cast<uint32>(price);
...
BUILDIN_FUNC(playerbot_buyingstoreopen2)
{
	const char* bot_key = script_getstr(st, 2);
	const char* title = script_getstr(st, 3);
	t_itemid nameid1 = static_cast<t_itemid>(script_getnum(st, 4));
	int32 amount1 = script_getnum(st, 5);
	int32 price1 = script_getnum(st, 6);
	t_itemid nameid2 = static_cast<t_itemid>(script_getnum(st, 7));
	int32 amount2 = script_getnum(st, 8);
	int32 price2 = script_getnum(st, 9);
	int32 zenylimit = script_getnum(st, 10);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.requested", "buyingstore", title, "operator.start", "ok", "", "");

	if (sd == nullptr || nameid1 == 0 || amount1 <= 0 || price1 <= 0 || nameid2 == 0 || amount2 <= 0 || price2 <= 0 || zenylimit <= 0 || nameid1 == nameid2) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "buyingstore", title, "target.invalid", "denied", "buyingstore.open2", sd == nullptr ? "bot.offline" : "buyingstore.invalid");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	int8 setup = buyingstore_setup(sd, 2);
	if (setup != 0) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "buyingstore", title, "target.invalid", "denied", "buyingstore.open2", ("setup=" + std::to_string(setup)).c_str());
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	PACKET_CZ_REQ_OPEN_BUYING_STORE_sub items[2] = {};
	items[0].itemId = static_cast<uint32>(nameid1);
	items[0].amount = static_cast<uint16>(amount1);
	items[0].price = static_cast<uint32>(price1);
	items[1].itemId = static_cast<uint32>(nameid2);
	items[1].amount = static_cast<uint16>(amount2);
	items[1].price = static_cast<uint32>(price2);

	int8 result = buyingstore_create(sd, zenylimit, 1, title, items, 2, nullptr);
	bool ok = (result == 0 && sd->state.buyingstore && sd->buyer_id != 0 && sd->buyingstore.slots >= 2);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, ok ? "interaction.completed" : "interaction.failed", "buyingstore", title, ok ? "operator.start" : "script.busy", ok ? "ok" : "aborted", ok ? "" : "buyingstore.open2", ok ? playerbot_buyingstore_state(sd).c_str() : ("result=" + std::to_string(result)).c_str());
	script_pushint(st, ok ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(playerbot_buyinglistopen)
{
	...
	if (!playerbot_buyinglist_refresh(sd, buyer_sd, true)) {
...
BUILDIN_FUNC(playerbot_buyingstoreclose)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.requested", "buyingstore", "close", "operator.stop", "ok", "", "");

	if (sd == nullptr) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.failed", "buyingstore", "close", "target.invalid", "denied", "buyingstore.close", "bot.offline");
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	if (!sd->state.buyingstore) {
		playerbot_trace_interaction(bot_id, char_id, account_id, sd, "interaction.completed", "buyingstore", "close", "operator.stop", "noop", "", "already.closed");
		script_pushint(st, 1);
		return SCRIPT_CMD_SUCCESS;
	}

	buyingstore_close(sd);
	bool ok = !sd->state.buyingstore;
	playerbot_trace_interaction(bot_id, char_id, account_id, sd, ok ? "interaction.completed" : "interaction.failed", "buyingstore", "close", ok ? "operator.stop" : "script.busy", ok ? "ok" : "aborted", ok ? "" : "buyingstore.close", playerbot_buyingstore_state(sd).c_str());
	script_pushint(st, ok ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_buyingstoreactive)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	script_pushint(st, (sd != nullptr && sd->state.buyingstore) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(playerbot_buyingstoresummary)
{
	const char* bot_key = script_getstr(st, 2);
	uint32 bot_id = 0, char_id = 0, account_id = 0;
	map_session_data* sd = playerbot_online_session_by_key(bot_key, &bot_id, &char_id, &account_id);
	script_pushstrcopy(st, playerbot_buyingstore_state(sd).c_str());
	return SCRIPT_CMD_SUCCESS;
}
...
/*==========================================
 * Add one waypoint to the in-memory route for a live headless BL_PC.
 *------------------------------------------*/
BUILDIN_FUNC(headlesspc_routeadd)
{
	uint32 char_id = script_getnum(st, 2);
	uint16 x = script_getnum(st, 3);
	uint16 y = script_getnum(st, 4);

	script_pushint(st, chrif_headlesspc_routeadd(char_id, x, y) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
{
	uint32 char_id = script_getnum(st, 2);
	bool loop = script_getnum(st, 3) != 0;

	script_pushint(st, chrif_headlesspc_routestart(char_id, loop) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(headlesspc_status)
{
	...
	script_pushint(st, chrif_headlesspc_status(char_id));
	return SCRIPT_CMD_SUCCESS;
}
...
/*==========================================
 * Query last completed headless spawn-ready ack sequence by char_id.
 *------------------------------------------*/
BUILDIN_FUNC(headlesspc_spawnack)
{
	uint32 char_id = script_getnum(st, 2);

	script_pushint(st, chrif_headlesspc_spawn_ack(char_id));
	return SCRIPT_CMD_SUCCESS;
}
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(playerbot_profile,"s"),
	...
	BUILDIN_DEF(playerbot_cartset,"si"),
	BUILDIN_DEF(playerbot_cartclear,"s"),
	BUILDIN_DEF(playerbot_cartgrant,"sii"),
	BUILDIN_DEF(playerbot_cartcount,"si"),
	BUILDIN_DEF(playerbot_cartactive,"s"),
	BUILDIN_DEF(playerbot_pcloaded,"s"),
	...
	BUILDIN_DEF(playerbot_buyingstoreopen2,"ssiiiiiii"),
