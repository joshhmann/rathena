...
e_refineui_attempt_result clif_refineui_attempt( map_session_data* sd, uint16 index, t_itemid material, bool use_blacksmith_blessing ){
#if PACKETVER < 20161012
	return REFINEUI_ATTEMPT_DENIED;
#else
	nullpo_retr( REFINEUI_ATTEMPT_DENIED, sd );
	...
	if( cost->chance >= ( rnd() % 10000 ) ){
		...
		return REFINEUI_ATTEMPT_SUCCESS;
	}
	...
	if( blacksmith_amount > 0 ){
		...
		return REFINEUI_ATTEMPT_FAILED;
	}
...
void clif_parse_refineui_refine( int32 fd, map_session_data* sd ){
	...
	uint16 index = server_index( p->index );
	clif_refineui_attempt( sd, index, p->itemId, p->blacksmithBlessing != 0 );
#endif
}
