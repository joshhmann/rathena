...
cmd_holder() {
	...
	# Check if it's a numeric bot_id
	if [[ "$holder" =~ ^[0-9]+$ ]]; then
		# Numeric: try as bot_id first
		local bot_exists
		bot_exists=$(query "SELECT 1 FROM bot_profile WHERE bot_id = '$holder' LIMIT 1" | tail -n +2)
		if [[ "$bot_exists" == "1" ]]; then
			where="holder_bot_id = '$holder'"
			resolved=1
		fi
	fi
	
	if [[ "$resolved" -eq 0 ]]; then
		# Try to resolve as bot_key
		local bot_id
