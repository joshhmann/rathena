...
cmd_bot() {
	...
	local sql="SELECT ts, action, IFNULL(NULLIF(controller_id,''), '-'), map_name, x, y, reason_code, result, IFNULL(NULLIF(error_code,''), '-'), char_id, 
			   IFNULL((SELECT p.bot_key FROM bot_profile p JOIN bot_identity_link l2 ON l2.bot_id = p.bot_id WHERE l2.char_id = bot_trace_event.char_id LIMIT 1), '-') 
			   FROM bot_trace_event 
			   WHERE $where 
			   ORDER BY id DESC 
			   LIMIT $n"
...
cmd_stats() {
	...
	# Total events
	local total
	total=$(query "SELECT COUNT(*) FROM bot_trace_event")
